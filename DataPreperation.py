...
class DataGenerator():
    def __init__(self, sr, n_mels, fmax, top_db, eps, seed, dataset_type="train", path=path):
        ...
        path_to_audio_p = list(map(lambda x: create_mel(x, self.sr, self.n_mels, self.fmax, self.top_db, self.eps), positive_paths))
        ...
        path_to_audio_n = list(map(lambda x: create_mel(x, self.sr, self.n_mels, self.fmax, self.top_db, self.eps), negative_paths))
